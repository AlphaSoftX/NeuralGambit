...
//   search::movehelpers - make/unmake wrappers that keep the NNUE accumulator
//                         stack in sync with the board; every make/unmake
//                         call in every section below goes through these
...
// Limits, staticEval() and iterativeDeepening().
...
// search::movehelpers — make/unmake wrappers that keep the NNUE
// accumulator stack (SearchContext::accStack) in sync with the board.
//
// Every make/unmake anywhere else in this file goes through these four
// functions instead of calling chess::Board's makeMove/unmakeMove
// directly — that indirection is what guarantees the accumulator can
// never silently drift out of sync with the board, no matter how many
// call sites touch it.

namespace search::movehelpers
{

  inline void doMove(search::SearchContext &ctx, int ply, const Move &m)
  {
    if (!ctx.nnueActive)
    {
    ...
    nnue::PerspectiveAccumulators &dst = ctx.accStack[ply + 1];
    dst = ctx.accStack[ply]; // start from the parent; patched incrementally below

    const Color mover = board.sideToMove();
    const Piece movingPiece = board.at(m.from());
    const bool isKingMove = movingPiece.type() == PieceType::KING;

    // Identify any captured piece BEFORE the move is played — board.at()
    // only tells the truth about this right now. Castling's "to" square
    // holds the mover's OWN rook (see below), which is never a capture,
    // so it's explicitly excluded here rather than misread as one.
    Piece capturedPiece = Piece::NONE;
    Square capturedSq = m.to();
    if (m.typeOf() == Move::ENPASSANT)
    {
      // The captured pawn sits one rank behind `to`, not on it.
      capturedSq = Square(m.to().index() + (mover == Color::WHITE ? -8 : 8));
      capturedPiece = board.at(capturedSq);
    }
    ...
    const int whiteKingSq = nnue::features::perspectiveKingSquare(board, Color::WHITE);
    const int blackKingSq = nnue::features::perspectiveKingSquare(board, Color::BLACK);

    if (capturedPiece != Piece::NONE)
      nnue::removePiece(dst, whiteKingSq, blackKingSq, capturedPiece.color(), capturedPiece.type(), capturedSq.index());

    if (m.typeOf() == Move::CASTLING)
    {
      const bool kingSide = m.to().index() > m.from().index();
      const int rank = m.from().index() / 8;
      const Square rookFrom = m.to();
      const Square rookTo = Square(rank * 8 + (kingSide ? 5 : 3));

      nnue::removePiece(dst, whiteKingSq, blackKingSq, mover, PieceType::ROOK, rookFrom.index());
      nnue::addPiece(dst, whiteKingSq, blackKingSq, mover, PieceType::ROOK, rookTo.index());
    }
    else if (m.typeOf() == Move::PROMOTION)
    {
      nnue::removePiece(dst, whiteKingSq, blackKingSq, mover, PieceType::PAWN, m.from().index());
      nnue::addPiece(dst, whiteKingSq, blackKingSq, mover, m.promotionType(), m.to().index());
    }
    else if (!isKingMove)
    {
      nnue::removePiece(dst, whiteKingSq, blackKingSq, mover, movingPiece.type(), m.from().index());
      nnue::addPiece(dst, whiteKingSq, blackKingSq, mover, movingPiece.type(), m.to().index());
    }

    board.makeMove(m);

    if (isKingMove || m.typeOf() == Move::CASTLING)
    {
      nnue::Accumulator &moverAcc = (mover == Color::WHITE) ? dst.white : dst.black;
      nnue::refreshOne(board, mover, moverAcc);
    }
  }

  // Reverts the board to the state before `m`
  inline void undoMove(search::SearchContext &ctx, const Move &m)
  {
    ctx.board.unmakeMove(m);
  }

  // Null move: no piece moves, so neither perspective's accumulator
  // changes at all
  inline void doNullMove(search::SearchContext &ctx, int ply)
  {
    if (ctx.nnueActive)
      ctx.accStack[ply + 1] = ctx.accStack[ply];
    ctx.board.makeNullMove();
  }

  inline void undoNullMove(search::SearchContext &ctx)
  {
...
namespace search::qsearch
{
  ...
  int quiescence(search::SearchContext &ctx, int alpha, int beta, int ply)
  {
    ...
    // (killers/history/accumulator stack are indexed by ply up to MAX_PLY
    // elsewhere).
    if (ply >= search::MAX_PLY - 1)
      return search::staticEval(ctx, ply);
    ...
    if (!inCheck)
    {
      standPat = search::staticEval(ctx, ply);
      if (standPat >= beta)
        return beta;
      if (standPat > alpha)
        alpha = standPat;
    ...
    for (const auto &m : moves)
    {
      ...
      movehelpers::doMove(ctx, ply, m);
      int score = -quiescence(ctx, -beta, -alpha, ply + 1);
      movehelpers::undoMove(ctx, m);
...
// in alphaBeta itself rather than living here — it recursively calls
// search() itself, which would make this namespace depend on alphabeta
...
namespace search::alphabeta
{
  ...
  {
    if (ply >= search::MAX_PLY - 1)
      return ctx.board.inCheck() ? 0 : search::staticEval(ctx, ply);
    ...
    int staticEvalScore = inCheck ? 0 : search::staticEval(ctx, ply);

    // Improving: is this node's static eval at least as good as the
    // static eval 2 plies ago
    bool improving = false;
    if (!inCheck)
    {
      ctx.staticEvalStack[ply] = staticEvalScore;
      if (ply >= 2 && ctx.staticEvalStack[ply - 2] != NO_STATIC_EVAL)
        improving = staticEvalScore >= ctx.staticEvalStack[ply - 2];
    }

    {
      int rfpScore;
      if (pruning::tryReverseFutility(inCheck, isPV, isRoot, depth, staticEvalScore, beta, improving, rfpScore))
        return rfpScore;
    }

    {
      int razorScore;
      if (pruning::tryRazoring(ctx, inCheck, isPV, isRoot, depth, staticEvalScore, alpha, beta, ply, razorScore))
        return razorScore;
    }
    ...
    //   - staticEvalScore < beta: the position doesn't even statically
    //     look good enough to deserve the assumption "passing the move is
    ...
    if (!inCheck && !isPV && nullMoveAllowed && depth >= 3 && !isRoot && staticEvalScore >= beta)
    {
      ...
      if (nonPawnPieces.count() >= 1)
      {
        ...
        // bigger reduction.
        int R = (depth >= 6 ? 3 : 2) + (staticEvalScore - beta >= 200 ? 1 : 0);
        movehelpers::doNullMove(ctx, ply);
        int nullScore = -search(ctx, depth - 1 - R, -beta, -beta + 1, ply + 1, false, extensions);
        movehelpers::undoNullMove(ctx);
        if (ctx.stop)
          return 0;
    ...
    bool canFutilityPrune = pruning::isFutile(inCheck, isPV, depth, staticEvalScore, alpha, improving);
...
namespace search
{
  ...
  void iterativeDeepening(SearchContext &ctx)
  {
    ...
    // Snapshot isLoaded()/isEnabled() ONCE for the whole search
    ctx.nnueActive = nnue::isLoaded() && nnue::isEnabled();

    // NNUE accumulator root refresh
    if (ctx.nnueActive)
      nnue::refresh(ctx.board, ctx.accStack[0]);
